...
class Application():
    def __init__(self):
        ...
        self.df_cpfs = pd.DataFrame(columns=['CPF'])  # DataFrame para armazenar os CPFs
        janela.mainloop()
    ...
    def exibir_tela_cpf(self):
        self.frame1.destroy()  
        formatacao_tela_cpf(self.janela, self.df_cpfs)

    def exibir_tela_emitir_cartao(self):
        self.frame1.destroy()
        formatacao_tela_emitir_cartao(self.janela, self.df_cpfs)
        
    def exibir_tela_memorando(self):
        self.frame1.destroy()
        formatacao_tela_emitir_memorando(self.janela, self.df_cpfs)

    def exibir_tela_trocar_senha(self):
        self.frame1.destroy()
        formatacao_tela_trocar_senha(self.janela, self.df_cpfs)

class formatacao_tela_emitir_cartao:

    def __init__(self, janela, df_cpfs):
        self.df_cpfs = df_cpfs
    ...
    def fazer_carteiras(self):
        ...
        os.path.isfile('banco_de_dados.xlsx')
        workbook = openpyxl.load_workbook('banco_de_dados.xlsx')
        ...
        for index, row in planilha.iterrows():
            ...
            # Verifique se a pasta correspondente à unloc já existe
                ...
                # Caminho completo para salvar o PDF na pasta correspondente
    ...
    def voltar_tela_principal(self):
        ...
        Application()  # Crie novamente a tela principal
...
class formatacao_tela_cpf:
    ...
    def consulta_cpfs(self):
        planilha = pd.read_excel(r"Y:\ARQUIVO DIGITAL CPR\fabio jr\teste\cpfs.xlsx")
        ...
        if cpf_digitado:
            ...
            if cpf_formatado in planilha['CPF'].values:
                ...
                try:
                    ...
                    planilha.to_excel(r"Y:\ARQUIVO DIGITAL CPR\fabio jr\teste\cpfs.xlsx", index=False)
...
class formatacao_tela_emitir_memorando:
    ...
    def botoes_tela_memorando(self):
        ...
        municipio_label = Label(self.janela, text="Município:", font=('Arial', 9,), bg='azure1', fg='black')
        municipio_label.place(relx=0.2, rely=0.33, anchor='n')
    ...
    def automacao_memorando(self):
        ...
        workbook = openpyxl.load_workbook(r"Y:\ARQUIVO DIGITAL CPR\fabio jr\Teste\banco_de_dados.xlsx")
        ...
        df = pd.read_excel(r"Y:\ARQUIVO DIGITAL CPR\fabio jr\Teste\banco_de_dados.xlsx")
        ...
        if selected_data.empty:
            mensagem = (f"Não foram encontrados dados para o município '{municipio}' na data '{data}'.")
        else:
            ...
            # Dividir a mensagem em linhas e inserir na Listbox
            for linha in mensagem.split('\n'):
                self.confirmacao.insert(END, linha)
        ...
        if municipio:
            # Verifique se a pasta correspondente à UNLOC já existe
            pasta_unloc = os.path.join(r"Y:\CARTEIRAS DIGITAIS", municipio)
